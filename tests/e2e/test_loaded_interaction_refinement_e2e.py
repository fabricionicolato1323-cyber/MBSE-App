...
def test_loaded_interaction_refinement_requires_target_and_explicit_communication(
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            # Existing interaction: intent -> friendly category -> explicit exchange.
            ...
            expect(page.get_by_text("Which exchange would you like to work on?", exact=True)).to_be_visible(
                timeout=20_000
            )
            page.get_by_role(
                "button",
                name=re.compile(r"^Kill info"),
            ).click()
            ...
            # Return from the exchange refinement to the intent-first loaded-model menu.
            page.get_by_role("button", name="Back to the interaction list", exact=True).click()
            expect(
                page.get_by_role("button", name="Add something new", exact=True)
            ).to_be_visible(timeout=20_000)

            # New interaction: intent -> friendly category, then normal source/target flow.
