"""Persistent dispatcher for the complete reviewed Level 1 SAM PoC.
...
from sam_level1_complete_incremental import (
    ...
    build_incremental_plan_complete,
...
def _prepare_manifest_for_relationship_plan(
    ...
    *,
    ...
    prepared = enrich_state_with_current_scenarios(
        model,
        scenarios=scenarios,
        state=prepared,
        settings=settings,
    ...
    return enrich_state_with_remaining_relationships(
        model,
        scenarios=scenarios,
        state=prepared,
        settings=settings,
...
def _adopt_current_managed_instance(
    ...
    *,
    ...
    adopted = enrich_state_with_current_scenarios(
        model,
        scenarios=scenarios,
        state=adopted,
        settings=settings,
...
def preview_level1_with_incremental_state(
    ...
    *,
    ...
    full = build_level1_sync_plan(
        model, scenarios=scenario_rows, project_id=settings.project_id
    )
    ...
    incremental = build_incremental_plan_complete(
        _with_manifest(model, prepared),
    ...
    return {
        **full,
        ...
        "library": {
            "action": "reuse",
            "package_name": (
                prepared.get("library_package_name") or ARCADIA_OA_LIBRARY_PACKAGE
            ),
        },
        "instance": {
            "action": "reuse",
            "package_name": prepared.get("instance_package_name"),
            "package_id": prepared.get("instance_package_id"),
        },
        "package_name": prepared.get("instance_package_name"),
...
def _finish_new_baseline(
    ...
    *,
    ...
    """Upgrade historical activity-only scenario trees in the same confirmed send."""
    if not [item for item in scenario_rows if item.get("valid") is not False]:
        value = dict(adopted)
        value["scenario_structure_revision"] = SCENARIO_STRUCTURE_REVISION
        return value

    upgrade = sync_level1_incremental_complete(
        _with_manifest(model, adopted),
        scenarios=scenario_rows,
        settings=settings,
    ...
    state = upgrade.get("sync_state")
    if not isinstance(state, dict):
        raise RuntimeError(
            "The initial Operational Scenario structure could not be finalized for incremental sync."
        )
    return state
...
def sync_level1_with_incremental_state(
    ...
    *,
    ...
    if manifest is None:
        ...
        if baseline is None:
            return {
                ...
                "snapshot_digest": adopted.get("snapshot_digest"),
                "package_name": adopted.get("instance_package_name"),
                "sam_package_id": adopted.get("instance_package_id"),
                "sync_state": adopted,
                "incremental_baseline_ready": True,
                "delta": {
                    "create": 0,
                    "update": 0,
                    "delete": 0,
                    "unchanged": len(_base._nodes_by_id(model)),
                },
                "relationship_delta": {
                    "create": 0,
                    "update": 0,
                ...
                "scenario_delta": {
                    ...
                    "delete": 0,
                    "unchanged": len(
                        [item for item in scenario_rows if item.get("valid") is not False]
                    ),
                },
            }
        ...
        baseline["scenario_delta"] = {
            "create": len([item for item in scenario_rows if item.get("valid") is not False]),
            "update": 0,
            "delete": 0,
            "unchanged": 0,
        }
        baseline["scenario_structure_revision"] = SCENARIO_STRUCTURE_REVISION
        return baseline
    ...
    result = sync_level1_incremental_complete(
        _with_manifest(model, prepared),
        scenarios=scenario_rows,
        settings=settings,
        expected_digest=expected_digest,
