import {PARTICIPANTS, state, minFor, buildModel, autoLayout, loadSaved, persist, descendants,
  expandAllContainers, minimumContainerDimensions, visibleLayoutEntries, storageKey} from './oa_diagram_v2_state.js';
...
function beginPan(event) {
  if (event.button !== 0 || event.target.closest('.oa-diagram-node, .oa-diagram-port, .oa-diagram-edge-label')) return;
...
function onPointerDownCapture(event) {
  ...
  if (node?.dataset.nodeId) { beginMove(event, node.dataset.nodeId); return; }
  beginPan(event);
}
...
function onPointerMoveCapture(event) {
  ...
  if (d.kind === 'move') {
    d.ids.forEach(id => { const p = state.layout.get(id), start = d.startPositions.get(id); if (p && start) { p.x = start.x + dx; p.y = start.y + dy; } });
    expandAllContainers(); updateAllNodes(); updateBounds(); renderEdges(); return;
  }
  if (d.kind === 'resize') {
    ...
    p.w = Math.max(minW, d.w + dx); p.h = Math.max(minH, d.h + dy); updateNode(d.id); updateBounds(); renderEdges();
  }
}
...
function onPointerEndCapture(event) {
  ...
  state.drag = null; el.viewport.classList.remove('is-panning'); expandAllContainers(); updateAllNodes(); updateBounds(); renderEdges(); persist();
}
...
function setModel(model, session) {
  ...
  if (nextSession !== state.session) {
    state.session = nextSession; state.view = {x: 28, y: 28, zoom: 1}; state.selected.clear();
