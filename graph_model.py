...
class OAGraph:
    ...
    def find_sid(self, sid: str) -> str | None:
        wanted = str(sid).strip()
        if not wanted:
            return None
        for node_id, data in self.graph.nodes(data=True):
            if str(data.get("sid", "")).strip() == wanted:
                return node_id
        return None
    ...
    def add_node(
        ...
        **attributes,
        ...
        supplied_uuid = attributes.pop("uuid", None)
        if supplied_uuid is not None:
            return False, "", "Element UUIDs are generated by the model and are immutable."

        sid = attributes.get("sid")
        if sid is not None:
            sid = str(sid).strip()
            if not sid:
                attributes.pop("sid", None)
            elif self.find_sid(sid):
                return False, "", f"Stable ID '{sid}' is already in the model."
            else:
                attributes["sid"] = sid
    ...
    def update_node_attributes(self, node_id: str, **attributes) -> bool:
        ...
        if "uuid" in attributes and str(attributes["uuid"]) != str(node_id):
            return False
        attributes.pop("uuid", None)

        if "sid" in attributes:
            current_sid = self.graph.nodes[node_id].get("sid")
            new_sid = str(attributes["sid"]).strip()
            if current_sid is not None and str(current_sid) != new_sid:
                return False
            duplicate_sid = self.find_sid(new_sid)
            if duplicate_sid is not None and duplicate_sid != node_id:
                return False
            attributes["sid"] = new_sid

        self._checkpoint()
        self.graph.nodes[node_id].update(attributes)
        self.graph.nodes[node_id]["uuid"] = str(node_id)
        return True
    ...
    def add_relation(
        ...
        **attributes,
        ...
        supplied_uuid = attributes.pop("uuid", None)
        if supplied_uuid is not None and not _is_uuid(supplied_uuid):
            return False, "Connection UUID must be a valid UUID."
        edge_uuid = str(supplied_uuid or uuid4())
    ...
    def _validate_imported_graph(self) -> None:
        for node_id, data in self.graph.nodes(data=True):
            node_type = data.get("type")
            if node_type not in NODE_TYPES:
                raise ValueError(f"Unsupported imported node type: {node_type!r}.")
            if str(data.get("uuid", "")) != str(node_id):
                raise ValueError("Imported node UUID does not match its graph identity.")
            if node_type in PARTICIPANT_TYPES:
                nature = data.get("nature", "unspecified")
                if nature not in PARTICIPANT_NATURES:
                    raise ValueError(f"Unsupported participant nature: {nature!r}.")
                if node_type == "OperationalActor" and nature != "human_individual":
                    raise ValueError(
                        "An imported Operational Actor must be one indivisible human participant."
                    )
                if node_type == "OperationalEntity" and nature == "human_individual":
                    raise ValueError(
                        "An imported human individual must be an Operational Actor."
                    )
        ...
        for source, target, data in self.graph.edges(data=True):
            ...
            if (source_type, relation, target_type) not in ALLOWED_RELATIONS:
                raise ValueError(
                    "Imported graph contains a relation that violates the model rules: "
                    f"{source_type} --{relation}--> {target_type}."
                )
            if not _is_uuid(data.get("uuid")):
                raise ValueError("Imported connection does not have a valid UUID.")

    @classmethod
    def load(cls, path: str | Path, *, allow_migration: bool = False) -> "OAGraph":
        source = Path(path)
        ...
        schema_version = int(loaded.graph.get("schema_version", 1) or 1)
        migration_needed = schema_version < MODEL_SCHEMA_VERSION
        migration_needed = migration_needed or any(
            not _is_uuid(node_id)
            or str(data.get("uuid", "")) != str(node_id)
            for node_id, data in loaded.nodes(data=True)
        )
        migration_needed = migration_needed or any(
            not _is_uuid(data.get("uuid"))
            for _, _, data in loaded.edges(data=True)
        )

        if migration_needed and not allow_migration:
            raise ValueError(
                "This model uses a legacy identity/schema. Re-run the load with "
                "allow_migration=True only after confirming the migration."
            )
        ...
        if migration_needed:
            ...
            if mapping:
                loaded = nx.relabel_nodes(loaded, mapping, copy=True)
                for old_id, new_id in mapping.items():
                    loaded.nodes[new_id].setdefault("sid", str(old_id))

            for node_id, data in loaded.nodes(data=True):
                data["uuid"] = str(node_id)

            for _, _, _, data in loaded.edges(keys=True, data=True):
                if not _is_uuid(data.get("uuid")):
                    data["uuid"] = str(uuid4())

        loaded.graph["model"] = "Arcadia Operational Analysis"
        loaded.graph["schema_version"] = MODEL_SCHEMA_VERSION
        loaded.graph["identity"] = "uuid"
        loaded.graph["migrated_from_legacy"] = bool(migration_needed)

        model = cls()
        model.graph = loaded
        model._history = []
        model._validate_imported_graph()
        return model

    def save(self, path: str | Path) -> Path:
        output = Path(path)
        output.parent.mkdir(parents=True, exist_ok=True)
        self.graph.graph["model"] = "Arcadia Operational Analysis"
        self.graph.graph["schema_version"] = MODEL_SCHEMA_VERSION
        self.graph.graph["identity"] = "uuid"
